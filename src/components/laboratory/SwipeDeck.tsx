...
import { IdeaCard } from '@/components/autopilot/IdeaCard';
import { UndoToast } from '@/components/autopilot/UndoToast';
...
export function SwipeDeck({ productId }: SwipeDeckProps) {
  ...
  const loadDeck = async () => {
    try {
      const res = await fetch(`/api/laboratory/${productId}/swipe/deck`);
      if (res.ok) {
  ...
  const handleSwipe = useCallback(async (action: SwipeAction, notes?: string) => {
    ...
    try {
      const res = await fetch(`/api/laboratory/${productId}/swipe`, {
        method: 'POST',
